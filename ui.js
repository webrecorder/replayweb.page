 ...
 * SPDX-License-Identifier: BSD-3-Clause
 */
class re extends ie{}re.directiveName="unsafeSVG",re.resultType=2;const oe=ee(re),ne=ee(class extends te{constructor(e){var t;if(super(e),e.type!==Y||"style"!==e.name||(null===(t=e.strings)||void 0===t?void 0:t.length)>2)throw Error("The `styleMap` directive must be used in the `style` attribute and must be the only part in the attribute.")}render(e){return Object.keys(e).reduce(((t,i)=>{const a=e[i];return null==a?t:t+`${i=i.replace(/(?:^(webkit|moz|ms|o)|)(?=[A-Z])/g,"-$&").toLowerCase()}:${a};`}),"")}update(e,[t]){const{style:i}=e.element;if(void 0===this.ut){this.ut=new Set;for(const e in t)this.ut.add(e);return this.render(t)}this.ut.forEach((e=>{null==t[e]&&(this.ut.delete(e),e.includes("-")?i.removeProperty(e):i[e]="")}));for(const e in t){const a=t[e];null!=a&&(this.ut.add(e),e.includes("-")?i.setProperty(e,a):i[e]=a)}return T}});var se=__webpack_require__(6274),le=__webpack_require__(3029),ce=__webpack_require__.n(le);const de=r(se.Z);function ue(e){return[de,e]}const he=window.IS_APP||window.electron&&window.electron.IS_APP||window.matchMedia("(display-mode: standalone)").matches,fe="1.5.5";function be(e){" "==e.key&&(e.preventDefault(),e.target.click())}class pe extends J{constructor(){super(),this.size="1.1em",this.width=null,this.height=null}static get properties(){return{svg:{type:String},size:{type:String},width:{type:String},height:{type:String}}}static get styles(){return o`
    :host {
    ...
    `}renderContent(){switch(this.state){case"googledrive":return L`<wr-gdrive .sourceUrl=${this.sourceUrl} @load-ready=${this.onLoadReady}/>`;case"started":return L`
          <div class="progress-div">
            ...
            ${this.currentSize&&this.totalSize?L`
              <p class="loaded-prog">Loaded <b>${Ue()(this.currentSize)}</b> of <b>${Ue()(this.totalSize)}</b></p>`:L``}
